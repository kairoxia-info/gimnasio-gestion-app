-- Bug crítico encontrado el 09/09/2026, reportado por Nalux: "a los alumnos
-- no se ve la imagen o video del ejercicio".
--
-- Causa: ver_plan_por_codigo() -- la RPC pública que arma el plan del
-- alumno -- busca la foto/video de cada ejercicio de la rutina con
--   SELECT e.media_url FROM public.ejercicios e
--    WHERE e.gimnasio_id = v_gimnasio_id AND e.id::text = ...
-- Esto NUNCA matchea un ejercicio de la biblioteca base (gimnasio_id IS
-- NULL, compartida entre todos los gimnasios -- ver migración de
-- ejercicios_biblioteca_global). Contado en la base antes de este fix: 500
-- ejercicios de la biblioteca base tienen media_url cargado, contra apenas 4
-- ejercicios propios por gimnasio -- así que para la enorme mayoría de las
-- rutinas armadas con ejercicios de la biblioteca, el alumno recibía
-- mediaUrl = NULL aunque el profesor sí lo viera bien al armar la rutina
-- (esa pantalla no tiene este filtro).
--
-- Arreglo: el WHERE pasa a aceptar el ejercicio propio del gimnasio O uno de
-- la biblioteca base, igual que ya hacen las consultas normales de
-- EjerciciosPage.jsx/RutinasPage.jsx (gimnasio_id = propio OR NULL).
-- CREATE OR REPLACE con el cuerpo completo porque no se puede "parchear"
-- solo una línea de una función ya creada -- el resto queda textualmente
-- igual a la versión anterior.
CREATE OR REPLACE FUNCTION public.ver_plan_por_codigo(p_codigo text)
 RETURNS TABLE(alumno_nombre text, gimnasio_nombre text, gimnasio_logo_url text, gimnasio_color_principal text, rutina_nombre text, rutina_descripcion text, rutina_duracion_semanas integer, rutina_fecha_inicio date, rutina_items jsonb, plan_nombre text, plan_notas text, plan_items jsonb, aviso_id uuid, aviso_titulo text, aviso_mensaje text, cuota_vence date, cuota_estado text, cuota_aviso_titulo text, cuota_aviso_mensaje text, rutina_restringida boolean, alimentacion_restringida boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- La copia guardada en la asignación manda, sin fallback: items nunca es
  -- NULL (NOT NULL DEFAULT '[]'::jsonb) y un array vacío es un dato legítimo
  -- (rutina asignada sin ejercicios todavía), no "faltan datos, mostrar la
  -- plantilla en vivo". El LEFT JOIN queda solo para nombre/descripción/
  -- duración de una fila vieja que de verdad no tenga copia (NULL, no '[]').
    ...
    FROM (
      SELECT
        ...
        t.item || jsonb_build_object(
          'mediaUrl',
          -- FIX (09/09/2026): antes solo "e.gimnasio_id = v_gimnasio_id",
          -- nunca matcheaba un ejercicio de la biblioteca base
          -- (gimnasio_id NULL, donde está casi toda la media cargada).
          (SELECT e.media_url
