-- ============================================================================
-- 0067_alias_mercadopago.sql
--
-- Pedido de Nalux (21/09/2026), visto en un video de un competidor (Control
-- Gym): hoy el alumno no tiene forma de saber a qué cuenta transferirle al
-- gimnasio -- se resuelve por afuera (WhatsApp, de palabra). Un campo de texto
-- en Configuración ("Alias de MercadoPago") que el alumno ve en su portal al
-- lado del recordatorio de cuota.
--
-- LO QUE NO ES, para que no se confunda después: esto NO es cobro automático
-- ni toca la API de Mercado Pago. Es un texto que el profesor carga a mano y
-- el alumno lee; el pago lo sigue confirmando el profesor como siempre
-- (Pagos → Registrar pago). Tampoco tiene nada que ver con la Fase 0 de
-- suscripción pendiente (esa es Kairox cobrándole al gimnasio; esto es el
-- alumno pagándole al gimnasio). No se valida que el alias exista de verdad.
--
-- Columna directa en `gimnasios`, sin RPC: `authenticated` ya tiene UPDATE
-- sobre la tabla entera (migración 0002, sin recorte por columna ni trigger
-- de congelamiento -- verificado el 21/09), con RLS que lo limita al propio
-- gimnasio. Mismo camino que nombre/color/días desde DatosGimnasio.jsx.
--
-- ver_plan_por_codigo() devuelve el alias siempre que esté cargado; es el
-- frontend el que decide cuándo mostrarlo (solo con la cuota vencida o con
-- deuda, ver MiPlanPage.jsx). DROP + CREATE porque cambia el RETURNS TABLE,
-- columna nueva AL FINAL, mismo criterio de siempre.
-- ============================================================================

ALTER TABLE public.gimnasios ADD COLUMN alias_mercadopago TEXT;

COMMENT ON COLUMN public.gimnasios.alias_mercadopago IS
  'Alias de Mercado Pago (texto libre, sin validar) que el alumno ve en su portal junto al recordatorio de cuota. NULL = no se muestra nada. No es integración con la API de MP.';
...
BEGIN
  ...
  -- Alias "g." (mismo motivo que el "a." de la 0062): alias_mercadopago es
  -- también columna del RETURNS TABLE, sin alias sería ambiguo.
