-- ============================================================================
-- 0056_fecha_fin_en_ver_plan_por_codigo.sql
--
-- 15/09/2026, pedido de Nalux: "que haya alertas cuando se vencen los
-- planes de rutina de ejercicio y de alimentación". Ya existía esta alerta
-- del lado del PROFESOR (tarjeta "Planes por vencer" en DashboardPage.jsx,
-- calculada directo sobre rutinas_asignadas.fecha_fin /
-- planes_alimentacion.fecha_fin) -- lo que faltaba era que el ALUMNO
-- también la viera en su propia pantalla, mismo criterio que ya existe para
-- la cuota (cuota_aviso_titulo/cuota_aviso_mensaje, más abajo en esta misma
-- función).
--
-- Encontrado de paso investigando esto: ver_plan_por_codigo() NUNCA
-- devolvió ni rutina_fecha_fin ni plan_fecha_fin -- MiPlanPage.jsx ya tenía
-- código (fechaFin={plan?.plan_fecha_fin}) esperando ese campo para el PDF
-- de alimentación, pero como la función nunca lo mandaba, siempre llegaba
-- undefined: el PDF que descarga el ALUMNO nunca mostraba la fecha de
-- vencimiento del plan, aunque el profesor le hubiera puesto una. Esta
-- migración corrige eso de paso, sin que nadie lo hubiera reportado
-- todavía.
--
-- DROP + CREATE porque cambia el RETURNS TABLE (agrega dos columnas al
-- final) -- eso borra los GRANTs, por eso se re-otorgan al final (mismo
-- patrón ya usado en migraciones anteriores de esta función).
-- ============================================================================
...
BEGIN
  ...
  SELECT pa.nombre, pa.notas, pa.fecha_fin, pa.items
    INTO v_plan_nombre, v_plan_notas, v_plan_fin, v_plan_items
  ...
  RETURN QUERY
    SELECT
      ...
      v_plan_nombre, v_plan_notas, v_plan_fin, v_plan_items,
