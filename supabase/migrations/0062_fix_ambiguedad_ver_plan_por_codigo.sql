-- ============================================================================
-- 0062_fix_ambiguedad_ver_plan_por_codigo.sql
--
-- Arreglo inmediato de un bug real de la migración anterior (0061), mismo
-- criterio que ya usó el proyecto para esto (0056 -> 0057): probado en local
-- apenas se aplicó 0061, ver_plan_por_codigo() rompía con "column reference
-- \"aceptacion_datos_en\" is ambiguous" (Postgres 42702).
--
-- Causa: las columnas nuevas del RETURNS TABLE (aceptacion_datos_en,
-- aceptacion_deslinde_en) se llaman EXACTAMENTE igual que las columnas
-- reales de public.alumnos -- en PL/pgSQL, las columnas de un RETURNS TABLE
-- se declaran como variables implícitas en todo el cuerpo de la función, así
-- que el SELECT sin alias de tabla ("SELECT ..., aceptacion_datos_en, ...
-- FROM public.alumnos") no podía saber si esas dos columnas eran la tabla o
-- la variable de retorno. El resto de las columnas nuevas de esa migración
-- (alumno_fecha_nacimiento) no tenía este problema por estar prefijadas
-- distinto del nombre real de columna (fecha_nacimiento) -- se mantienen los
-- nombres de retorno tal cual (no hace falta re-prefijarlos, el frontend ya
-- quedó escrito esperando aceptacion_datos_en/aceptacion_deslinde_en), y se
-- agrega el alias "a." al SELECT que los lee para desambiguar.
--
-- DROP + CREATE porque toca el cuerpo de la función (mismo RETURNS TABLE de
-- la 0061, sin cambios de columnas).
-- ============================================================================
...
BEGIN
  ...
  -- Alias "a." a propósito (el fix de esta migración): sin él,
  -- aceptacion_datos_en/aceptacion_deslinde_en son ambiguos contra las
  -- columnas del RETURNS TABLE de arriba, que se llaman exactamente igual.
