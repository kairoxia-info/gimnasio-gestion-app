-- Fix encontrado en revisión (04/09/2026), no lo reportó Nalux con un error:
-- la migración 0026 dejó un fallback en ver_plan_por_codigo() que rompe la
-- garantía que esa misma migración vino a dar ("al alumno que ya se le
-- asignó, que le quede como está").
--
-- rutinas_asignadas.items es NOT NULL DEFAULT '[]'::jsonb -- nunca puede
-- faltar (la 0026 le hizo backfill a todas las filas viejas, y toda fila
-- nueva se crea con snapshotRutina(), que siempre manda items). El CASE de
-- la 0026 ("si la copia viene vacía, mostrar la plantilla en vivo") por eso
-- nunca hacía falta como red de seguridad -- y de paso rompía justo el caso
-- que se quería evitar: si alguna vez se asigna una rutina que en ese
-- momento tiene 0 ejercicios (se está armando, o se le sacaron todos para
-- reordenarla) y DESPUÉS se le cargan ejercicios a la plantilla, el alumno
-- que la tenía asignada empezaba a ver ese contenido nuevo en vez de seguir
-- con lo que tenía (o nada, si no tenía nada) -- exactamente lo que 0026
-- prometió que no iba a pasar más.
