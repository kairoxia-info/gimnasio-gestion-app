-- Ajuste pedido por Nalux (03/09/2026), en la misma conversación que armó
-- 0020: la política "restringir" NO tiene que apagar la rutina de
-- ejercicios y el plan de alimentación juntos, como un solo interruptor --
-- son dos cosas separadas y el profesor tiene que poder decidir cada una
-- por su cuenta. Por default, solo se restringe la rutina (el caso que
-- pidió originalmente); el plan de alimentación queda visible salvo que el
-- profesor prenda esa casilla a propósito.
--
-- Reemplaza acceso_restringido (0020, un solo booleano) por dos: una casilla
-- por tipo de contenido, ambas leídas dentro de ver_plan_por_codigo() para
-- decidir, cada una por separado, si esa parte viaja en la respuesta.
ALTER TABLE public.gimnasios
  ADD COLUMN restringir_rutina BOOLEAN NOT NULL DEFAULT true,
  ADD COLUMN restringir_alimentacion BOOLEAN NOT NULL DEFAULT false;
