-- ============================================================================
-- 0070_como_pagar_whatsapp_email.sql
--
-- Punto 3 del pedido de Nalux (21/09/2026): el portal del alumno pasa a
-- tener un menú lateral (el mismo cajón del profesor) con una sección nueva,
-- "Cómo pagar". Ahí va el alias de Mercado Pago (que sale de adentro de la
-- tarjeta de cuota, donde vivía desde la 0067 -- la tarjeta queda solo con el
-- estado) más dos datos nuevos y opcionales para que el alumno sepa a dónde
-- mandar el comprobante después de transferir: un WhatsApp y/o un correo del
-- gimnasio. Los carga el profesor en Configuración, al lado del alias. Si no
-- están, no se muestran.
--
-- Texto libre las dos, sin validar (mismo criterio que el alias). Para que el
-- link de WhatsApp abra el chat, el número tiene que ir con código de país;
-- el campo lo aclara y el frontend se queda solo con los dígitos.
--
-- Columnas directas en gimnasios, sin RPC (mismo motivo que en la 0067:
-- authenticated ya tiene UPDATE sobre la tabla, con RLS al propio gimnasio).
-- ver_plan_por_codigo() las devuelve al final (DROP + CREATE, cuerpo
-- idéntico a la 0069 salvo las dos columnas).
-- ============================================================================

ALTER TABLE public.gimnasios
  ADD COLUMN whatsapp_pagos TEXT,
  ADD COLUMN email_pagos    TEXT;

COMMENT ON COLUMN public.gimnasios.whatsapp_pagos IS
  'WhatsApp del gimnasio para que el alumno mande el comprobante de pago (texto libre, con código de país; el portal usa solo los dígitos). NULL = no se muestra.';
COMMENT ON COLUMN public.gimnasios.email_pagos IS
  'Correo del gimnasio para que el alumno mande el comprobante de pago (texto libre). NULL = no se muestra.';
...
DECLARE
  ...
  v_gim_color     TEXT;
  v_gim_alias_mp  TEXT;
  v_gim_wsp_pagos TEXT;
  v_gim_mail_pagos TEXT;
  v_noticias      JSONB;
  v_rut_nombre    TEXT;
  v_rut_desc      TEXT;
...
BEGIN
  ...
  SELECT g.nombre, g.logo_url, g.color_principal, g.alias_mercadopago, g.whatsapp_pagos, g.email_pagos,
    ...
    INTO v_gim_nombre, v_gim_logo, v_gim_color, v_gim_alias_mp, v_gim_wsp_pagos, v_gim_mail_pagos,
  ...
  RETURN QUERY
    SELECT
      ...
      v_noticias,
      NULLIF(btrim(v_gim_wsp_pagos), ''),
      NULLIF(btrim(v_gim_mail_pagos), '');
END;
$function$;
